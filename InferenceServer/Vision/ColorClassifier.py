...
# ⚠ 2026-05-15 — 파스텔(저채도) 알약 대응:
#   화이트밸런스 후 흰 알약은 잔여 cyan(H≈88~150)·저S(≤32) 로
#   몰리므로, 따뜻한색(빨강/주황/노랑/분홍) 의 S_lo 를 낮춰도
#   흰색을 침범하지 않는다. 반면 cyan 영역의 파랑/초록/보라 는
#   S_lo 를 높게 유지해 흰 알약 오분류를 막는다.
HSV_CATEGORIES = [
    # ----- 따뜻한 컬러 (파스텔 대응 — S_lo 낮춤) -----
    ("빨간색",    0,    7,   50,  255,   60,  255),
    ("빨간색",  165,  179,   50,  255,   60,  255),
    ("주황",      8,   25,   40,  255,   80,  255),
    ("노란색",   26,   35,   35,  255,   80,  255),
    # R4: 연두 S_lo 35→28 — 파스텔 연두(#2 H43 S32)가 흰색으로
    #     빠지던 문제. 흰 알약은 H≈98~104 라 연두 H 36~55 와
    #     hue 대역이 안 겹쳐 안전.
    ("연두",     36,   55,   28,  255,   60,  255),
    ("분홍",      0,   12,   22,   80,  150,  255),
    ("분홍",    160,  179,   22,   80,  150,  255),
    # 차가운 조명이 연분홍을 자홍·저채도로 밀어냄. 측정상 분홍 hue
    # 가 사진/각도에 따라 H≈122~155 로 흔들림(불안정). 흰 알약은
    # 잔여 cyan H≈98~104 라, 그보다 위(H 112~165)·저채도 영역을
    # 분홍으로 판정. S_hi 49 로 채도 있는 파랑/보라 침범 차단.
    # R4: 하한 135→112 (이 사진 #5 H122 S19 포착).
    ("분홍",    112,  165,   10,   49,  150,  255),
    ("갈색",      0,   25,   60,  255,   30,  130),
    # ----- 차가운 컬러 (흰색 보호 — S_lo 높게 유지) -----
    ("초록",     56,   85,   55,  255,   40,  255),
    ("파란색",   86,  130,   55,  255,   40,  255),
    ("보라",    131,  159,   50,  255,   40,  255),
    # ----- 무채색 (낮은 S, fallback) -----
...
class ColorClassifier:
    ...
    def classify_array(crop_bgr: np.ndarray,
        ...
        # ⚠ 화이트밸런스는 YoloDetector 에서 '전체 이미지' 기준으로
        #   이미 적용됨(crop 단위 WB 는 알약을 무채색화해 노랑·분홍을
        #   흰색으로 만들던 문제). 여기선 보정본을 그대로 분석.
